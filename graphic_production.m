function graphic_production(N,rn,rd,r0d,r1d,tchar,tspan,R,Rn,H,Acytoplasm,cAtyp,nItyp,cRtyp,cPtyp,cKtyp,cCtyp)
...
pastel_factor=0.5;
cmap=(1-pastel_factor)+pastel_factor*cmap; 
...
outurd=A(:,18*N+20:19*N+22); % cell displacement
outwrd=A(:,19*N+23:20*N+25); % ECM displacement
...
for k=1:length(time)
    cGtot(k)=2*pi*trapz(rn,outcGd(k,:).*rn')/Acytoplasm; % total fraction actin monomers
    cFtot(k)=2*pi*trapz(rn,outcFd(k,:).*rn')/Acytoplasm; % total fraction actin filaments
    cSptot(k)=2*pi*trapz(rn,outcSpd(k,:).*rn')/Acytoplasm; % total fraction actin in stress fibres
    ...
    cRAtot(k)=2*pi*trapz(rn,outcRAd(k,:).*rn')/Acytoplasm; % total fraction of ROCK activated
    cPPtot(k)=2*pi*trapz(rn,outcPPd(k,:).*rn')/Acytoplasm; % total fraction of MLCP phosphorylated
    cKPtot(k)=2*pi*trapz(rn,outcKPd(k,:).*rn')/Acytoplasm; % total fraction of MLCK phosphorylated
    cCPtot(k)=2*pi*trapz(rn,outcCPd(k,:).*rn')/Acytoplasm; % total fraction of cofilin phosphorylated
end
...
text(0.8, 1.03, 'Cell radius', 'Units','normalized','Interpreter','latex','HorizontalAlignment','center', 'VerticalAlignment','bottom','FontSize',20);
hold on
...
text(0.8, 1.03, 'Global biochemical state', 'Units','normalized','Interpreter','latex','HorizontalAlignment','center', 'VerticalAlignment','bottom','FontSize',20);
xlim([0 tinset])
ylim([0 100])
for i=2:length(tspan)
...
text(0.8, 1.03, 'Focal adhesions', 'Units','normalized','Interpreter','latex','HorizontalAlignment','center', 'VerticalAlignment','bottom','FontSize',20);
ylim([0 inf])
hold on
hold on
ax_inset = axes('Position',[.2 .575 .25 .25]);  % store handle
box on
nangular=N;
...
axis equal
text(ax_inset, 0.5, 1.04, '$t = 10000\,\mathrm{s}$', ...
    'Units','normalized', ...
    'Interpreter','latex', ...
...
text(0.8, 1.03, 'ROCK', 'Units','normalized','Interpreter','latex','HorizontalAlignment','center', 'VerticalAlignment','bottom','FontSize',20); 
hold on
ax_inset = axes('Position',[.2 .4 .25 .25]);
box on
nangular=N;
theta=linspace(0,2*pi,nangular);
cRAdend=outcRAd(end,:);
...
axis equal
text(ax_inset, 0.5, 1.04, '$t = 10000\,\mathrm{s}$','Units','normalized','Interpreter','latex','HorizontalAlignment','center', 'VerticalAlignment','bottom','FontSize',16);
...
text(0.8, 1.03, 'Actin filaments', 'Units','normalized','Interpreter','latex','HorizontalAlignment','center', 'VerticalAlignment','bottom','FontSize',20);
ylim([0 0.9])
hold on
ax_inset=axes('Position',[.2 .6 .25 .25])
box on
nangular=N;
theta=linspace(0,2*pi,nangular);
cFdend=outcFd(end,:);
...
text(0.8, 1.03, 'Ventral stress fibres', 'Units','normalized','Interpreter','latex','HorizontalAlignment','center', 'VerticalAlignment','bottom','FontSize',20);
hold on
ax_inset = axes('Position',[.2 .325 .25 .25])
box on
nangular=N;
theta=linspace(0,2*pi,nangular);
cSpdend=outcSpd(end,:);
...
colororder(timeplottingcolors)
xlabel('$r/d_{c}$','Interpreter','Latex','FontSize',36)
ylabel('$u_{r}^{(c)}/d_{c}$','Interpreter','Latex','FontSize',36)
title('(a)','Interpreter','Latex','FontSize',36)
text(0.8, 1.03, 'Cell displacement', 'Units','normalized','Interpreter','latex','HorizontalAlignment','center', 'VerticalAlignment','bottom','FontSize',20);
ylim([-inf 7.5e-3])
hold on
...
ylabel('$\partial{u_{r}^{(c)}}/\partial r \: (\%)$','Interpreter','Latex','FontSize',24)
ax_inset=axes('Position',[.2 .23 .25 .25])
box on
nangular=N;
theta=linspace(0,2*pi,nangular);
...
x_circle=r_circle*cos(theta);
y_circle=r_circle*sin(theta);
Rn=rd(1); % radius of the occluded inner region
ur0c=urdend(1); % boundary value u_{r0}^{(c)}
nr_inner=80; % resolution for inner region
...
colororder(timeplottingcolors)
xlabel('$r/d_{c}$','Interpreter','Latex','FontSize',36)
ylabel('$\sigma_{C}^{(c,r)}/E_{0}^{F}$','Interpreter','Latex','FontSize',36)
title('(c)','Interpreter','Latex','FontSize',36)
text(0.8, 1.03, 'Cell radial stress', 'Units','normalized','Interpreter','latex','HorizontalAlignment','center', 'VerticalAlignment','bottom','FontSize',20);
ylim([0 inf])
hold on
ax_inset=axes('Position',[.6 .625 .25 .25])
box on
nangular = N;
theta = linspace(0, 2*pi, nangular);
[R,Theta] = meshgrid(rd, theta);
...
colorbar;
hold on
Rn=rd(1); % radius of the nucleus
ur0c=urdend(1); % boundary value at r=Rn
nr_inner=80; % radial resolution
r_inner=linspace(0,Rn,nr_inner);
theta_inner=linspace(0,2*pi,nangular);
[R_in, Theta_in] = meshgrid(r_inner, theta_inner);
X_in = R_in .* cos(Theta_in);
Y_in = R_in .* sin(Theta_in);
stress_inner=10*(1/Rn)*ur0c*ones(size(R_in));
...
%% radial displacement ECM
figure(13)
plot(rd,outwrd(timeplot(2:end),2:N+2),'LineWidth',2.25)
set(gca,'fontsize',24)
colororder(timeplottingcolors)
xlabel('$r/d_{c}$','Interpreter','Latex','FontSize',36)
ylabel('$w_{r}^{(c)}/d_{c}$','Interpreter','Latex','FontSize',36)
title('(b)','Interpreter','Latex','FontSize',36)
text(0.8, 1.03, 'ECM displacement', 'Units','normalized','Interpreter','latex','HorizontalAlignment','center', 'VerticalAlignment','bottom','FontSize',20);
hold on
...
hold on
xticks([0.2 0.3 0.4 0.5])
ylabel('$\partial w^{(c)}_{r}/\partial r \: (\%)$','Interpreter','Latex','FontSize',24)
fig = gcf;
fig.Units = 'inches';
fig.Position = [0 0 8.5 6.5];       
fig.PaperUnits = 'inches';
...
colororder(timeplottingcolors)
xlabel('$r/d_{c}$','Interpreter','Latex','FontSize',36)
ylabel('$\sigma_{E}^{(c,r)}/E_{0}^{F}$','Interpreter','Latex','FontSize',36)
title('(d)','Interpreter','Latex','FontSize',36)
text(0.8, 1.03, 'ECM radial stress', 'Units','normalized','Interpreter','latex','HorizontalAlignment','center', 'VerticalAlignment','bottom','FontSize',20);
hold on
fig = gcf;
fig.Units = 'inches';
fig.Position = [0 0 8.5 6.5];       
