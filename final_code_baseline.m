...
v0=1; % stall speed myosin motors

Acytoplasm=pi*(R^2-Rn^2);
...
% initial conditions
cG0=100*ones(N+1,1); % actin monomers
cF0=zeros(N+1,1); % actin filaments 
cSp0=zeros(N+1,1); % VSFs
cM0=30*ones(N+1,1); % myosin II
cMp0=0*ones(N+1,1); % activated myosin II
nf0=100*ones(N+1,1); % free integrins
nh0=zeros(N+1,1); % high-affinity integrins
nb0=zeros(N+1,1); % bound integrins
nA0=zeros(N+1,1); % FAs

% ligand patterning
NS0=1000; % ligand density
ns0=NS0*ones(N+1,1); % ligands

% signaling proteins 
delta=4; % strength of increase in response to FA formation
cR0=1e-3*ones(N+1,1); % ROCK
cRA0=0*ones(N+1,1); % Activated ROCK 
cP0=1*ones(N+1,1); % MLCP
cPP0=0*ones(N+1,1); % MLCP-P
cC0=1*ones(N+1,1); % cofilin
cCP0=0*ones(N+1,1); % phosphorylated cofilin
cK0=1e-1*ones(N+1,1); % MLCK
cKP0=0*ones(N+1,1); % MLCK-P 

% rate/diffusion constants

% structural/scaffolding proteins
Kpp=2e-3; % actin polymerisation rate
Kpm=1e-2; % actin depolymerisation rate
Kap=1e-2; % myosin activation rate
Kam=1e-2; % myosin inactivation rate
KFp=5e1; % adhesion maturation rate
KFm=1e-3; % focal adhesion disassembly rate

kmp=1e-4; % myosin binding rate
kmm=1e-2; % myosin unbinding rate
khp=0.5; % free integrin activation rate
khm=5; % high-affinity integrin inactivation rate
kbp=1e-4; % nascent adhesion formation rate
kbm=1e-2; % nasecent adhesion disassembly rate
...
% signaling proteins
KRp=1e-2; % ROCK activation rate
kRm=1e-1; % activated ROCK inactivation rate
K1p=1e-2;  % MLCP phosphorylationg rate
k1m=1e-2; % MLCP-P dephosphorylation rate
K2p=1e-2; % MLCK phosphorylation rate
k2m=1e-2; % MLCK-P dephosphorylation rate
K3p=5e-2; % cofilin phosphorylation rate
k3m=1e-2; % phosphorylated cofilin dephosphorylation rate
...
nI0bar=(2/(R^2-Rn^2))*trapz(rn,(nf0+nh0+nb0+nA0).*rn); % integrins
...
% dimensionless rate/diffusion constants
...
Eldcell=fFd+ElS0d*fSd+ElP0d*fPd; % Young's modulus cell cytoplasm
EldECM=ElC0d*fCd+ElO0d*fOd; % Young's modulus ECM
mueffd=mu0d*g1d+Fd*hd/v0d; % effective viscosity cytoplasm
muECMd=muE0d*g2d; % viscosity ECM
Eldadh=ElB0d*fBd+ElA0d*fAd; % stiffness per unit volume of adhesions
bBadhd=bB0d*fBd+bA0d*fAd; % drag per unit volume of adhesions

% displacements 
ur0(:,1)=zeros(N+3,1); % cell (dimensional)
wr0(:,1)=zeros(N+3,1); % ECM (dimensional)

ur0d(:,1)=ur0/L; % cell (dimensionless)
wr0d(:,1)=wr0/L; % ECM (dimensionless)

% stress initialisation
sigmacell=zeros(N+1,1); % stress cell (dimensionless)
sigmaECM=zeros(N+1,1); % stress ECM (dimensionless)
lambda=zeros(N+1,1); % adhesion stretch
kfa=lambda;

%% Actin treadmilling (dimensional and dimensionless)

U0=1e-2; % maximum treadmilling speed

U1=1+tanh(20*(rd(:)-r1d))-tanh(20*(rd(:)-r0d)); % treadmilling profile

U0sd=U0/vchar;
U1d=U0sd*U1; % dimensionless treadmilling speed
...
for j=1:length(tspan)-1
    ...
    % cytoplasm and ECM radial strain
    straincell(j+1,:)=100*[(-3*urd(2)+4*urd(3)-urd(4))/2/hrd;(urd(4:N+2)-urd(2:N))/2/hrd;(3*urd(N+2)-4*urd(N+1)+urd(N))/2/hrd];
    strainECM(j+1,:)=100*[(-3*wrd(2)+4*wrd(3)-wrd(4))/2/hrd;(wrd(4:N+2)-wrd(2:N))/2/hrd;(3*wrd(N+2)-4*wrd(N+1)+wrd(N))/2/hrd];
...
graphic_production(N,rn,rd,r0d,r1d,tchar,tspan,R,Rn,H,Acytoplasm,cAtyp,nItyp,cRtyp,cPtyp,cKtyp,cCtyp)

%% functions
    function fval=axisymmetric_differential_equations_solver(t,y,N)

        cGd=y(1:N+1); % actin monomers
        cFd=y(N+2:2*N+2); % actin filaments
        cSpd=y(2*N+3:3*N+3); % VSFs 
        cMd=y(3*N+4:4*N+4); % inactivated myosin II
        cMpd=y(4*N+5:5*N+5); % activated myosin II
        nfd=y(5*N+6:6*N+6); % free integrins
        nhd=y(6*N+7:7*N+7); % high affinity integrins 
        nbd=y(7*N+8:8*N+8); % bound integrins 
        nAd=y(8*N+9:9*N+9); % focal adhesions 
        nsd=y(9*N+10:10*N+10); % ligands
        cRd=y(10*N+11:11*N+11); % inactivated ROCK
        cRAd=y(11*N+12:12*N+12); % activated ROCK
        cPd=y(12*N+13:13*N+13); % MLCP
        cPPd=y(13*N+14:14*N+14); % MLCP-P
        cCd=y(14*N+15:15*N+15); % cofilin
        cCPd=y(15*N+16:16*N+16); % phosphorylated cofilin
        cKd=y(16*N+17:17*N+17); % MLCK
        cKPd=y(17*N+18:18*N+18); % MLCK-P
        urd=y(18*N+19:19*N+21); % cell dispalcement 
        wrd=y(19*N+22:20*N+24); % ECM displacement

        %% signalling proteins    
        kRp=KRp*(nbd(:)+delta*nAd(:)); % ROCK activation enhanced by adhesion maturation
        kRpt=kRp*tchar;

        dcRdt(:)=-kRpt(:).*cRd(:)+kRmt*cRAd(:);
        dcRAdt(:)=kRpt(:).*cRd(:)-kRmt*cRAd(:);

        k1p=K1p*cRAd(:); % phosphorylation of MLCP, MLCK, and cofilin depends on activated ROCK concentration 
        k2p=K2p*cRAd(:);
        k3p=K3p*cRAd(:);
        ...
        % actin polymerisation/depolymerisation rate dependencies
        kpp=Kpp*cRAd;
        kpm=Kpm*cCd;
        
        % myosin II activation/inactivation rate dependencies
        kap=Kap*cKPd;
        kam=Kam*cPd;
        ...
        % actin filaments 
        zeta1=cFd(:).*U1d(:).*rd(:); % treadmilling velocity term
        ...
        % non-diffusive/advective quantities
        dcSpdt(:)=kmpMAt*(cFd(:).*cFd(:).*cMpd(:)+cFd(:).*cSpd(:).*cMpd(:))-kmmt*cSpd(:); % stress fibres
        dnhdt(:)=khpt*nfd(:)-khmt*nhd(:)-kbpSt*nhd(:).*nsd(:)+kbmt*nbd(:); % high-affinity integrins
        dnbdt(:)=kbpSt*nhd(:).*nsd(:)-kbmt*nbd(:)-kFpt(:).*nbd(:)+kFmt(:).*nAd(:); % bound integrins 
        dnAdt(:)=kFpt(:).*nbd(:)-kFmt(:).*nAd(:); % focal adhesions 
        dnsdt(:)=-kbpNt*nhd(:).*nsd(:)+kbmNdSt*nbd(:); % ECM ligands
        ...
        fBd=nbd;
        fAd=nAd;
        Eldcell=fFd+ElS0d*fSd+ElP0d*fPd; % cell Young's modulus 
        EldECM=ElC0d*fCd+ElO0d*fOd; % ECM Young's modulus
        Eldadh=ElB0d*fBd+ElA0d*fAd; % adhesion stiffness per unit volume
        bBadhd=bB0d*fBd+bA0d*fAd; % adhesion drag per unit volume 
        hd=cSpd; % contractile stress dependence on VSF concentration
        mueffd=mu0d*g1d+Fd*hd/v0d; % effective viscosity cytoplasm
     
        gamma=0.05; % membrane tension constant (dimensional)    
        gammad=gamma/ElF0/L; % membrane tension constant (dimensionless)
        
        % formulate mechanical problem as M*[u,w]=V where M is the mass matrix
        V1r(1)=Eldnuc*urd(2)/rd(1)-Eldcell(1)*(urd(3)-urd(1))/2/hrd-Fd*hd(1); % BC
        ...
        V1r(N+3)=-Eldcell(N+1)*(urd(N+3)-urd(N+1))/2/hrd-Fd*hd(N+1)+gammad*(1/(rd(N+1)+urd(N+1))-1/rd(N+1)); % BC

        V2r(1)=EldECM(1)*wrd(2)/rd(1)-EldECM(1)*(wrd(3)-wrd(1))/2/hrd; % BC
        ...
        V2r(N+2)=(-(3*EldECM(N+1)-4*EldECM(N)+EldECM(N-1))/4/hrd/hrd-EldECM(N+1)./rd(N+1)/2/hrd-EldECM(N+1)/hrd/hrd).*wrd(N+3)...
            ...
            -Eldadh(N+1).*urd(N+2);
        V2r(N+3)=(1+alpha*alpha)*EldECM(N+1)*wrd(N+2)/rd(N+1)/(1-alpha*alpha)-EldECM(N+1)*(wrd(N+3)-wrd(N+1))/2/hrd; % BC
        
        % cell 
        M2r(1,1)=-mueffd(1)/2/hrd; % BC
        M2r(1,3)=mueffd(1)/2/hrd; % BC
        ...
        % cell 
        M2r(N+2,N+1)=-(3*mueffd(N+1)-4*mueffd(N)+mueffd(N-1))/4/hrd/hrd-mueffd(N+1)/rd(N+1)/2/hrd+mueffd(N+1)/hrd/hrd; % BC
        M2r(N+2,N+2)=-mueffd(N+1)/rd(N+1)/rd(N+1)-2*mueffd(N+1)/hrd/hrd-bBadhd(N+1); % BC
        M2r(N+2,N+3)=(3*mueffd(N+1)-4*mueffd(N)+mueffd(N-1))/4/hrd/hrd+mueffd(N+1)/rd(N+1)/2/hrd+mueffd(N+1)/hrd/hrd; % BC
        ...
        M2r(2*N+6,2*N+4)=-muECMd(N+1)/2/hrd; % BC
        M2r(2*N+6,2*N+5)=-(1+alpha*alpha)*muECMd(N+1)/rd(N+1)/(1-alpha*alpha); % BC
        M2r(2*N+6,2*N+6)=muECMd(N+1)/2/hrd; % BC
